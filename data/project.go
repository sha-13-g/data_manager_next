...
func GetProjectByAll() []ProjectDetails {
	...
	query := fmt.Sprintf(`SELECT p.id, p.name, p.description, cs.name cs_name, cs.logo, cs.branch, cs.contact, s.name s_name, c.name
	...
	for r.Next() {
		r.Scan(&project.Project_id, &project.Project_name, &project.Description, &project.Customer_name, &project.Customer_logo, &project.Customer_branch,
			&project.Customer_contact, &project.Service_name, &project.Commercial_name)
		projects = append(projects, project)
	}
